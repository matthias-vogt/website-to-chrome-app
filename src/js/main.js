...
var app = {
	...
	changedName: function() {
		...
		setTimeout(function() {
			...
			if (name && !$('#desc').val() ||
				$('#desc').val() === earlyVal + settings.descAppendix)
				$('#desc').val(name + settings.descAppendix);
		});
	},
	...
	urlChange: function() {
		...
		if (url)
			$('#window iframe').attr('src', url.match('//') ? url : '//' + url);
	...
	init: function() {
		...
		$('#url')
			.on('keydown change', debounce(app.urlChange, 500))
			.on('blur', function() {
				// add "//" if not in value
				if (!$(this).val().match('//'))
					$(this).val('//' + $(this).val())
